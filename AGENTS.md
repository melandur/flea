...
- `ui/Theme.qml` owns the singleton palette, type and spacing tokens from the Omarchy
  theme plus the user override, or from one of `ui/js/Themes.js`'s vendored palettes when the
  Theme section has chosen one; it also owns `markInk`, the one place a filetype colour is
  lifted to its WCAG floor. See "The theme catalog" and "The filetype tier".
...
- `vendor/` is third-party data the generators read and nothing imports at runtime, with the licence
  of each beside it; `vendor/README.md` states what crosses over and what does not.
- `ui/js/Keymap.js` is the generated key-to-action lookup and imports no QML.
- `ui/js/FileTypes.js` is what a filename says about its row beyond the backend's icon name: the
  mark families, the longest-first extension walk and the colour lookup, see "The filetype tier".
- `ui/js/FileTypeColors.js` is GENERATED by `tools/flea-filetypes-gen` from `vendor/yazi-icons.toml`
  and is the colour table alone, one packed line per colour. Never edit it; regenerate.
- `ui/js/Themes.js` is GENERATED by `tools/flea-themes-gen` from `vendor/strata-themes.toml`: the 95
  vendored palettes, and `paletteBody`, which emits one as the colors.toml text `ui/Theme.qml`
  already takes, see "The theme catalog". Never edit it; regenerate.
- `ui/js/SettingsTheme.js` is the Theme section's rows, and `ui/js/SettingsDisplay.js` the Display
  section's; both came out of `ui/js/Settings.js` at its file budget, the way `SettingsShelf.js` did.
- `ui/js/Tokens.js` is the metrics contract as one string, which `ui/Ipc.qml` serves as `tokens()`
  and `tools/flea-metrics-gate` diffs. It came out of `ui/Theme.qml` at that file's 400-line hard
  cap and is one job: report the resolved tokens, as against resolve them.
...
Settings > Display > Appearance > "File type marks and colours", `display.fileTypeColors`, off by
default. On, a row's mark and its ink come from the row's NAME rather than from the freedesktop
icon name the backend sends, which is as fine as `generic-icons` can be and no finer: it answers
`text-x-generic` for `.json`, `.toml`, `.yaml` and `.md` alike, and `application-x-generic` for
the 190 typeless `application/*` types "Icons in the row" counts.

**The two halves come from two places, and the seam is deliberate.** The COLOUR is yazi's, which is
nvim-web-devicons' table (MIT, © 2023 nvim-tree), vendored verbatim at `vendor/yazi-icons.toml` and
generated into `ui/js/FileTypeColors.js` by `tools/flea-filetypes-gen`: 713 rules over 217 colours,
485 extensions, 214 exact filenames and 14 directory names. The MARK is Flea's own, the lucide set
recut to the Omarchy edge in `ui/js/Icons.js`. **Nothing of yazi's geometry crosses over**, because
its `text` fields are nerd-font codepoints and "The row mark: Shape, not a generated font" already
ruled a font glyph out of this tree; the Network group's `+` was replaced for the same reason.
There are 282 distinct glyphs in that table and most of them are brand logos, so a `.rs` row draws
lucide's chevrons in devicons' rust orange and never a Rust logo: reproducing a trademark is a
different decision from importing a colour, and "The Network row marks" below is where this project
makes that one.

**`conds` is dropped on purpose.** Yazi's twelve conditional rules colour a link, an orphan, an
executable, a directory and the rest, and `ui/Theme.qml` already takes those from the Omarchy
palette's own roles, each lifted to a WCAG ratio against the running background. A vendored hex
would fight the theme on exactly the rows the theme is most specific about. A symlink therefore
keeps the link mark and the theme's symlink colour whatever it points at, which is the rule
`Icons.glyphForRow` already stated for the mark alone.

**`ui/js/FAMILIES` is deliberately coarser than the colour table.** A family exists only where the
SHAPE carries something the generic file mark loses; every other name keeps the mark the icon name
already chose and takes only the colour. A directory short-circuits before any lookup: the folder
mark is the whole of what says a row can be entered, and no extension may take that away. A
directory does read yazi's `dirs` table for its colour, which is where Downloads and `.config` are.

**Fifteen marks were added to `PATHS` for it**, lucide-static 1.38.0 again, ISC, the same source and
the same cut as the twelve row marks: `braces`, `code-xml`, `binary`, `database`, `disc`, `key`,
`book-open`, `palette`, `box`, `git-branch`, `hash`, `captions`, `mail`, `calendar`, `notebook`.
**The gate was the cut's own rule rather than the AE pixel compare**, which a deliberate recut
cannot pass by construction: "square the corner arcs, keep the extents". Each lucide original and
each recut string were rendered at 240x240 with `rsvg-convert` and their trimmed extents compared
with `magick -trim`, and all fifteen hold their source box inside the growth a SquareCap and a
MiterJoin cost at a corner, 5 px of 240 at the worst (`braces`, `code-xml`, `key`); eight are exact.
Where a curve is genuine it stays, cut rule 4: `disc` is two circles, `database` keeps its `A9 3`
skirts and its ellipse, `palette`'s body is curved the whole way round and only its four filled
`r=.5` dots became the zero-length-line dots `server` already draws. `braces` is the opposite case
and keeps no arc at all, because the shape is nothing but corners.

**The gate that matters in the tree is `tests/js/filetypes.js`**: every family must name a mark
`PATHS` carries AND resolve to something other than `pathFor("file")`, because a name missing from
`PATHS` falls back to the file mark in silence, which on a listing reads as a screen of blank
documents. That is the trap `drive`, `history` and `lock` each fell into before.

**Compound extensions are matched longest-first.** Yazi writes `d.ts`, `spec.tsx` and `blade.php` as
single keys, so `FileTypes.extensions` offers every suffix of a dotted name with the longest first
or `ts` would always beat `spec.ts`. A dotfile with no second dot is a name and not an extension,
which is why the run starts after the first character.

**`ui/Theme.qml markInk` is the only consumer of a vendored hex**, and it lifts each to the 3:1 a
graphical object needs on the running background before any board sees it, the same lift
`accentFrame` takes. It memoises, because the lift is a binary search plus up to 255 stepped
rounds and a scroll asks for the same dozen colours repeatedly; `applyColors` empties the memo,
which is the only thing that can invalidate it. **No board grew a line for any of this**: the four
that draw a row mark (`Row.qml`, `ColumnRow.qml`, `GridTile.qml`, `PreviewColumn.qml`) pass the row
itself as `glyphForRow`'s third argument, or `null` when the setting is off, and hand their
existing ink to `markInk` as its fallback. `ui/Row.qml` is at its ledger length and `PreviewColumn`
at its own, so that was a constraint and not a preference. The preview column takes the mark and
not the ink for the same reason.

### The theme catalog

Settings > Theme, `display.theme` in `src/uischema.rs`, `"omarchy"` by default. Following the
desktop is what Flea has always done and what the README promises; a catalog id replaces the
palette and nothing else. The catalog is Strata's 95 bundled themes (MIT, © 2026 LGSE Ltd.), most
of them Tinted Base16 imports, vendored at `vendor/strata-themes.toml` and generated into
`ui/js/Themes.js` by `tools/flea-themes-gen`.

**Eight of Strata's fourteen tokens cross over**, and the generator's header carries the mapping.
The load-bearing two are `syntax_string` to `green` and `syntax_preprocessor` to `cyan`: Flea's
symlink and executable roles read `cyan`/`color6` and `green`/`color2` off an Omarchy palette, and a
Strata theme has no ANSI ring at all. Its own `docs/themes.md` records the Base16 mapping it used,
`base0B` to strings and `base0C` to preprocessor, and those ARE that ring's green and cyan under
another name, so nothing is invented. `highlight`, `border`, `syntax_keyword`, `syntax_constant`
and `syntax_type` have no Flea role and are dropped, which is what `applyColors` already does with
an Omarchy theme's extra keys.

**`Themes.paletteBody` emits colors.toml text, not a structure.** That is the whole seam: a catalog
palette is handed to the same `applyColors` the desktop's own file goes through, so it runs the
identical parse, the same surface ladder, the same WCAG floors and the same assignments, and cannot
drift into a second code path. `Themes.resolve` is the one place that decides what a stored value
means, so an id a newer build wrote and this catalog does not carry follows the desktop rather than
leaving a window on a palette of fallbacks; the panel and `ui/Theme.qml` cannot disagree about it.

**Two bugs were found by running it, and both are the reason the code reads as it does.**

**One: `onThemeIdChanged` can run before a binding derived from `themeId` re-evaluates.** The first
version asked a `followingOmarchy` property inside `applyColors`; that property still held the old
answer when the handler ran, so choosing a theme stored it and left the window on the desktop's
palette. `applyColors` now ASKS `Themes.following(root.themeId)` outright. Reproduced on the box,
2026-09-18.

**Two: `qs.Commons Color` keeps its OWN `FileView` on the desktop's `theme/colors.toml`**, at
`Color.qml`'s `colorsFile`, with `onLoaded: root.loadColors(text())`. So `Color.loadColors(body)` is
not the last word: Color's own asynchronous re-read lands after `applyColors` and puts the
desktop's `background`, `foreground` and `accent` back. Those three are the only roles Flea binds
through rather than assigns, so the symptom was a catalog theme applying to `surface`, `muted`,
`symlink`, `executable`, `error` and `accentFrame` and to nothing else — measured through
`ui/Ipc.qml palette()` as `#1a1b26 #3a3432 #a9b1d6 ...`, three of the desktop's own in a row of
3024's. `Theme.color` now reads `root.catalogColors` for those three and falls through to `Color`
when it is null, which keeps following the desktop exactly what it always was and does not fight
Color's reload. Verified after the fix on the 3024 palette: all nine roles resolve to that theme,
`#090300 #3a3432 #a5a2a2 #807d7c #01a0e4 #dd372a #b5e4f4 #01a252 #01a0e4`, the `error` and the two
ladder colours carrying their lifts; and with `"omarchy"` stored, all nine come back to the
desktop's own.

**What a catalog theme does NOT replace is `shell.toml`.** Type, spacing, corner rounding and the
`Style` fills derived from them stay Omarchy's, because Strata states none of them and inventing
them is not importing them. A selection fill therefore keeps the desktop's tint under a catalog
palette; that is a known limit and not an oversight.

**95 rows are a `check` each, with the palette in the id.** `ui/js/SettingsTheme.js` builds them and
`ui/ViewState.qml changeSetting` answers a `theme:` id with a leaf write rather than a toggle, so
the catalog needed no new row kind on `ui/SettingsRow.qml` or `ui/SettingsPanel.qml`, both of which
are over their own caps already. Choosing a row is choosing that theme; there is no off.
